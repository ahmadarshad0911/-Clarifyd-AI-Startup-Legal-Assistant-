...
SYSTEM_PROMPT = (
    ...
    "SEVERITY RUBRIC — judge each clause on its merits and pick the band that best fits. "
    "Do NOT inflate ordinary, market-standard clauses (net-30 payment, mutual NDA, "
    "standard governing law, for-convenience termination with notice) — those are low. "
    "Escalate to a higher band ONLY when an ESCALATION TRIGGER below is present:\n"
    ...
    "ESCALATION TRIGGERS — if any apply, severity is AT LEAST high, and critical when "
    "exposure is uncapped, perpetual, or personal:\n"
    "  · uncapped / unlimited liability or indemnity\n"
    "  · a liability cap so low it gives no real protection (a nominal/token sum, e.g. $100)\n"
    "  · personal guarantee or founder personal liability\n"
    "  · assignment of pre-existing or unrelated IP, or IP assigned in perpetuity/irrevocably\n"
    "  · unilateral right for the counterparty to terminate, change terms, or set-off at will\n"
    "  · waiver of all claims, or one-sided obligation with no reciprocal duty\n"
    "DO NOT escalate for inconvenience that is curable with timely notice "
    "(e.g. auto-renewal with a non-renewal window, standard notice periods) — "
    ...
    "  Uncapped / unlimited indemnity → critical / 9-10.\n"
    "  Assignment of ALL IP incl. pre-existing/unrelated, in perpetuity → critical / 9-10.\n"
    "  Counterparty may terminate for convenience on short notice → high / 7-8.\n"
    "  Auto-renewal with a curable non-renewal notice window → medium / 5-6 (not high).\n"
