...
/**
 * Findings — risky clauses, loopholes, suggested replacements + collaborator
 * document export. Shows every analyzed draft (whether negotiated yet or not).
 *
 * The Negotiate page becomes a slim history view of drafts whose
 * `negotiated_at` has been stamped (which happens the first time the user
 * accepts a suggested clause or generates a collaborator doc here).
 */
...
function FindingsPageInner() {
  ...
  function applySuggestionsToText(): { revised: string; missed: string[] } | null {
    ...
    suggestions.forEach((s, i) => {
      ...
      if (flexible.test(revised)) { revised = revised.replace(flexible, replacement); return; }
      const loophole = loopholes[i];
      const loopTarget = (loophole?.excerpt || "").trim();
      if (loopTarget && revised.includes(loopTarget)) { revised = revised.replace(loopTarget, replacement); return; }
  ...
  async function exportDocument() {
    ...
    try {
      if (active.analysis.extracted_text) {
        ...
        if (result) {
          ...
          if (result.missed.length) {
            push(`Applied ${chosen.length - result.missed.length}/${chosen.length} — some clauses not found verbatim.`, "info", `Unmatched: ${result.missed.join(", ")}`);
          } else {
            push("Collaborator document ready", "success", `${chosen.length} clause${chosen.length === 1 ? "" : "s"} swapped — rest unchanged.`);
          }
          return;
        }
      }
      ...
      push("Ultimate collaborator document drafted", "success", "Counsel review still recommended. Re-upload to get verbatim-splice mode.");
    } catch (err) {
    ...
    return (
      <AppShell>
        <section className="crystal-glass rounded-3xl p-10 text-center flex flex-col items-center gap-4">
          <span className="material-symbols-outlined text-primary/30 text-[56px]">description</span>
          <h2 className="font-display-hero text-h1-mobile lg:text-h1 text-onboarding-navy m-0">No analysis yet</h2>
          <p className="text-on-surface-variant max-w-md">
            Upload a contract on the dashboard — Kimi will analyze every clause and the risky
            clauses + suggested replacements will appear here.
          </p>
  ...
  return (
    <AppShell>
      {exporting ? (
        <OrbitalLoader
          fullscreen
          statusLines={["Merging agreed clauses…", "Drafting collaborator document…", "Locking signature blocks…"]}
        />
      ) : null}
      ...
      <section className="crystal-glass rounded-3xl p-5 md:p-6">
        <div className="flex items-center justify-between gap-4 mb-4 flex-wrap">
          ...
          <Link href="/dashboard" className="btn-capsule glass-semi-clear text-primary text-sm px-5">
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block font-semibold text-body-sm truncate max-w-[200px]">{d.file_name}</span>
                  <span className={`block text-[11px] ${selected ? "text-white/80" : "text-on-surface-variant"}`}>
                ...
                <button
                  ...
                  className={`absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full inline-flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 ${
                    selected ? "bg-white/25 hover:bg-white/40 text-white" : "bg-status-danger/10 hover:bg-status-danger/20 text-status-danger"
                  }`}
                >
        ...
        <>
          ...
          <section className="flex flex-col gap-6">
            <div className="crystal-glass rounded-3xl p-6 md:p-8 relative overflow-hidden">
              <div className="absolute -top-24 -left-20 w-64 h-64 rounded-full pointer-events-none"
                style={{ background: "rgba(53, 37, 205, 0.12)", filter: "blur(70px)" }} aria-hidden />
              <div className="absolute -bottom-24 -right-16 w-56 h-56 rounded-full pointer-events-none"
                style={{ background: "rgba(124, 58, 237, 0.12)", filter: "blur(70px)" }} aria-hidden />
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-baseline gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="font-display-hero text-h2 text-onboarding-navy m-0">flagged</span>
                ...
                <div className="flex flex-col items-end gap-3">
                  <div className="flex items-center gap-3">
                    <div className="flex items-baseline gap-1">
                      <span key={`picked-${activePicked.size}`} className="count-pop font-display-hero text-3xl text-status-success leading-none transition-colors">
                        {activePicked.size}
                      </span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button type="button" onClick={applyAll} disabled={!suggestions.length}
                      className="btn-capsule glass-semi-clear text-primary text-sm px-4">
                      <span className="material-symbols-outlined text-[16px]">done_all</span>
                      Accept all
                    </button>
                    <button type="button" onClick={clearPicks} disabled={!activePicked.size}
            ...
            {loopholes.length === 0 ? (
              ...
              <ul className="flex flex-col gap-6 m-0 p-0 list-none">
                {loopholes.map((lp, i) => {
                  ...
                  const sev = (["low", "medium", "high", "critical"].includes(rawSev) ? rawSev : "low") as RiskLevel;
                  const meta = RISK_META[sev] ?? RISK_META.low;
                  const sg = suggestions[i];
                  const isPicked = activePicked.has(i);
                  return (
                    <ScrollReveal key={i} as="li" delay={Math.min(i, 4) * 80}
                      className={`risky-card ${isPicked ? "is-applied" : ""} ${meta.aura} p-5 md:p-7`}>
                      <>
                        <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
                          <div className="flex items-start gap-4 min-w-0">
                            <div className="shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-bold"
                              style={{ color: meta.ringColor, background: `${meta.ringColor}1a`, boxShadow: `inset 0 0 0 1px ${meta.ringColor}33` }}>
                              {meta.glyph}
                            </div>
                            <div className="min-w-0">
                              <h3 className="font-display-hero text-h3 md:text-2xl text-onboarding-navy m-0 leading-tight">{lp.clause_name}</h3>
                              <div className="flex flex-wrap items-center gap-2 mt-1.5">
                                <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${meta.chip}`}>{sev}</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-5 md:gap-7 items-stretch">
                          <div className="min-w-0">
                            <span className="font-label-caps text-label-caps uppercase text-on-surface-variant block mb-2">Risky clause</span>
                            ...
                            <p className="text-body-sm text-on-surface-variant mt-3 m-0"><strong className="text-onboarding-navy">Issue: </strong>{lp.issue}</p>
                            <p className="text-body-sm text-on-surface-variant mt-1 m-0"><strong className="text-onboarding-navy">Impact: </strong>{lp.impact}</p>
                          </div>
                          <div className="hidden md:flex flex-col items-center justify-center px-1">
                            <div className="flex-1 w-px bg-on-surface-variant/15" />
                            <span className="morph-arrow my-3"><span className="material-symbols-outlined text-[18px]">east</span></span>
                            <div className="flex-1 w-px bg-on-surface-variant/15" />
                          </div>
                          <div className="min-w-0">
                            <span className="font-label-caps text-label-caps uppercase text-status-success block mb-2">Suggested clause</span>
                            {sg ? (
                              <>
                                <p className="rounded-2xl p-4 bg-status-success/5 border-l-2 border-status-success/40 text-body-sm text-on-surface m-0 leading-relaxed">{sg.suggested_clause}</p>
                                <p className="text-[12px] text-on-surface-variant mt-3 m-0"><strong className="text-status-success">Why: </strong>{sg.rationale}</p>
                              </>
                            ) : (
                              <p className="text-body-sm text-on-surface-variant m-0">No suggestion produced.</p>
                            )}
                          </div>
                        </div>
                        {sg ? (
                          <div className="mt-6 flex justify-end">
                            <button type="button" onClick={() => togglePick(i)}
                              className={`btn-capsule text-sm px-7 ${isPicked ? "bg-status-success text-white border-transparent shadow-lg shadow-status-success/30" : "btn-capsule-primary"}`}
                              style={{ transition: "transform 0.45s cubic-bezier(0.34, 1.56, 0.64, 1), box-shadow 0.45s cubic-bezier(0.34, 1.56, 0.64, 1), background 0.3s ease" }}>
                              <span className="material-symbols-outlined text-[18px]" style={isPicked ? { fontVariationSettings: "'FILL' 1" } : undefined}>
            ...
            {loopholes.length ? (
              <div className="shimmer-gold-border rounded-3xl bg-onboarding-navy text-white p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative overflow-hidden">
                <div className="absolute -top-24 -right-24 w-72 h-72 rounded-full pointer-events-none"
                  style={{ background: "rgba(180, 83, 9, 0.25)", filter: "blur(80px)" }} aria-hidden />
                <div className="relative z-10">
                  <span className="font-label-caps text-label-caps uppercase tracking-widest text-onboarding-gold/90 block">Ultimate collaborator document</span>
                ...
                <div className="relative z-10 flex flex-wrap gap-3 w-full md:w-auto">
                  <button type="button" onClick={exportDocument} disabled={exporting || activePicked.size === 0}
